---
title: "[Spring] SSE를 통한 실시간 통신 서비스 도입 "
categories:
  - spring
---
...
여기서 사용자가 서비스를 종료한 뒤에 발생한 알림 이벤트들은 **`SSE`가 종료된 시점과 사용자가 서비스를 다시 연결하여 `SSE`가 다시 시작된 시점**을 비교하여 그 사이의 이벤트들을 전송해주는 것입니다.

아래에서 어떻게 구현했는지 말씀드리겠습니다.
